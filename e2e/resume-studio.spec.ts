...
test('critical resume studio flow is keyboard accessible',async({page})=>{
 ...
 await expect(page.getByText('TypeScript')).toBeVisible();await page.getByRole('button',{name:'Confirm'}).click();await page.getByRole('button',{name:'Generate'}).click();await expect(page.getByText('Make the summary clearer')).toBeVisible();
 await page.getByRole('button',{name:'Apply to draft'}).click();await page.getByLabel('Source').selectOption('manual');await page.getByLabel('Job title').fill('Staff Engineer');await page.getByLabel('Company').fill('Northstar');await page.getByLabel('Job description').fill('We need a TypeScript engineer who builds accessible products and reliable systems.');await page.getByLabel('Generate').selectOption('cover_letter');await page.getByRole('button',{name:/generate grounded draft/i}).click();
 await expect(page.getByText(/Cover Letter · version 1/)).toBeVisible();await page.getByRole('radio',{name:/Modern preview/}).press('Enter');await page.getByRole('button',{name:'Approve'}).click();const download=page.waitForEvent('download');await page.getByRole('button',{name:'PDF'}).click();expect((await download).suggestedFilename()).toBe('resume.pdf');
});
