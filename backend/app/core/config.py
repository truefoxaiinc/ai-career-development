...
class Settings(BaseSettings):
    ...
    @property
    def job_provider_targets(
        self,
    ) -> dict[str, list[str]]:

        parsed = json.loads(
            self.job_provider_targets_json
            or "{}"
        )
    ...
    def validate_runtime(
        ...
        if (
            self.jwt_secret.startswith(
                "development-only-change-me"
            )
            or len(self.jwt_secret) < 32
        ):
            raise RuntimeError(
                "JWT_SECRET must be a "
