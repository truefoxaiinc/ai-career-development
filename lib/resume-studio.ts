...
export const studioApi={
 ...
 resumeStatus:(id:string)=>api.get<StudioResume>(`/resume-studio/resumes/${id}`),
 retryResume:(id:string)=>api.post<StudioResume>(`/resume-studio/resumes/${id}/retry`),
 deleteResume:(id:string)=>api.delete<{deleted:boolean}>(`/resume-studio/resumes/${id}`),
 ...
 documents:()=>api.get<Array<{id:string;document_type:'resume'|'cover_letter';version:number;title:string;content:string;template_key:string;approved_at?:string|null;created_at:string;source_file_id?:string|null;claim_report:{unsupported_claims:number}}>>('/documents'),
